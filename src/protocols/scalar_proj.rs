...
use crate::utils::curve::{ZpElement, GtElement, ConvertToZp, G1Element, Zero};
...
impl ReduceLR for ScalarProjCm {
    fn reduce_l(&self, challenges: &Vec<ZpElement>) -> ZpElement {
...
impl ScalarProjCmProof for ScalarProjCm {
...
pub trait ScalarProjCmProof: ReduceLR {
        ...
        Mat<T>: 'static + BraKetZp, 
    {
        ...
        let la = mat_a.bra_zp(&self.get_l_vec());
        ...
        let mut r_current = self.get_r_vec()[0..n].to_vec();
        ...
        let mut l_current = self.get_l_vec()[0..m].to_vec();
...
mod tests {
    ...
    fn test_scalar_proj() {
        ...
        let scalar_proj_protocol = ScalarProjCm::new(
            c_com,
            a_com, 
            (m, n), 
            &l_vec, 
        ...
        scalar_proj_protocol.prove::<i64>(
            &srs, 
            &mut trans_seq, 
            &a,
            &a_cache);

        let result = scalar_proj_protocol.verify(&srs, &mut trans_seq);
