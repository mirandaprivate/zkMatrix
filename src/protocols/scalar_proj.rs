...
impl ScalarProjInterface for ScalarProj {
    fn reduce_l(&self, challenges: &Vec<ZpElement>) -> ZpElement {
...
impl ScalarProjInterface for ScalarProjPoly {
    fn reduce_l(&self, challenges: &Vec<ZpElement>) -> ZpElement {
...
impl ScalarProjCmProof for ScalarProj {}
impl ScalarProjRmProof for ScalarProj {}
impl ScalarProjCmProof for ScalarProjPoly {}
impl ScalarProjRmProof for ScalarProjPoly {}

pub trait ScalarProjCmProof: ScalarProjInterface {
    ...
    fn verify_as_subprotocol_cm(
        ...
        if let (
            ...
            let rhs = 
                a_reduce * x * xi_l * xi_r * srs.g_hat * srs.h_hat
        ...
        println!("!! Verification of ScalarProjCm failed");
        return false;
...
/// The row major protocol is equivalent to a column major protocol
/// on the transposition of the matrix.
/// Hence, we only modify the column major protocol such that
/// l_vec and r_vec are swapped.
/// h_vec and g_vec are swapped.
/// m and n are swapped.
/// Bra and Ket are swapped.
/// 
pub trait ScalarProjRmProof: ScalarProjInterface {
        ...
        Mat<T>: 'static + BraKetZp, 
    {
        ...
        for j in 0..log_m {
            ...
            let l_tr = 
                ...
                + u_0 * x * dirac::inner_product(&v_left, &l_right);
            let r_tr = 
                dirac::inner_product(&capital_a_right, &g_left)
                + u_0 * x * dirac::inner_product(&v_right, &l_left);
        ...
        for j in 0..log_n {
            ...
            let l_tr = 
                g_reduce * dirac::inner_product(&a_left, &h_right)
                + u_0 * x * l_reduce 
                * dirac::inner_product(&a_left, &r_right);
            let r_tr = 
                g_reduce * dirac::inner_product(&a_right, &h_left)
                + u_0 * x * l_reduce 
                * dirac::inner_product(&a_right, &r_left);
    ...
    fn verify_as_subprotocol_rm(
        ...
        if (
            ...
            println!("!! Invalid public input when verifying ScalarProjRm");
            return false;
        } 
            ...
            trans_seq.data[pointer_old + 4] 
        {
            ...
            println!("!! Invalid transcript when verifying ScalarProjRm");
            return false;
        }
        ...
        let n = self.get_shape().1;
        if m & (m - 1) != 0 || n & (n - 1) != 0 {
            panic!("Invalid shape when verifying ScalarProjRm");
        }
        let log_m = (m as u64).ilog2() as usize;
        let log_n = (n as u64).ilog2() as usize;
        ...
        if let (
            ...
            if lhs == rhs {
                ...
                println!(" pointer * {:?}", trans_seq.pointer);

                let check_1 = pip_g1.verify_as_subprotocol(srs, trans_seq);

                println!(" * {:?}", trans_seq.pointer);
                let check_2 = pip_g2.verify_as_subprotocol(srs, trans_seq);
                println!(" * {:?}", trans_seq.pointer);
        ...
        println!("!! Verification of ScalarProjRm failed");
        return false;
        
    }

    fn verify_rm(&self, srs: &SRS, trans_seq: &mut TranSeq
    ) -> bool {

        if trans_seq.check_fiat_shamir() == false {
            println!("!! Fiat shamir check failed when verifying ScalarProjRm");
            return false;
        }

        self.verify_as_subprotocol_rm(srs, trans_seq)
    }
...
mod tests {
    ...
    fn test_scalar_proj() {
        ...
        let scalar_proj_protocol = ScalarProj::new(
            c_com,
            a_com, 
            (m, n), 
            &l_vec, 
        ...
        scalar_proj_protocol.prove_rm::<i64>(
            &srs, 
            &mut trans_seq_rm, 
            &a,
            &a_cache_rm
        ...
        let result_rm = scalar_proj_protocol.verify_rm(
            &srs, &mut trans_seq_rm
        );

        assert_eq!(result_rm, true);

        println!(" * Verification of ScalarProjRm passed");

        let y = ZpElement::rand();

        let mut scalar_proj_poly = ScalarProjPoly::new(
            GtElement::zero(),
            a_com, 
            (m, n), 
            y,
        ...
        let y_l = scalar_proj_poly.get_l_vec();
        let y_r = scalar_proj_poly.get_r_vec();

        let ay = a.braket_zp(&y_l, &y_r);
        let ay_com = ay * srs.g_hat * srs.h_hat;

        scalar_proj_poly.c_com = ay_com;


        let mut trans_seq_cm = TranSeq::new();

        scalar_proj_poly.prove_cm::<i64>(
            &srs, 
            &mut trans_seq_cm, 
            &a,
            &a_cache_cm);

        let result_cm = scalar_proj_poly.verify_cm(
            &srs, &mut trans_seq_cm
        );

        assert_eq!(result_cm, true);

        println!(" * Verification of ScalarProjCmPoly passed");


        let mut trans_seq_rm = TranSeq::new();

        scalar_proj_poly.prove_rm::<i64>(
            &srs, 
            &mut trans_seq_rm, 
            &a,
            &a_cache_rm);

        let result_rm = scalar_proj_poly.verify_rm(
            &srs, &mut trans_seq_rm
        );

        assert_eq!(result_rm, true);

        println!(" * Verification of ScalarProjRmPoly passed");
